...
export interface PornographySetupData {
  sessionsPerDay: number;
  minutesPerSession: number;
}

interface PornographySetupModalProps {
  onComplete: (data: PornographySetupData) => void | Promise<void>;
  isSubmitting?: boolean;
}

export const PornographySetupModal = ({ onComplete, isSubmitting = false }: PornographySetupModalProps) => {
  const [step, setStep] = useState(1);
  const [sessionsPerDayInput, setSessionsPerDayInput] = useState("0");
  const [minutesPerSessionInput, setMinutesPerSessionInput] = useState("0");

  const sessionsPerDay = useMemo(() => Math.max(0, Number(sessionsPerDayInput) || 0), [sessionsPerDayInput]);
  const minutesPerSession = useMemo(
    () => Math.max(0, Number(minutesPerSessionInput) || 0),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-[#09090b] text-white shadow-2xl animate-scale-in">
        <div className="bg-gradient-to-r from-violet-600/80 to-indigo-600/80 p-8 text-center">
          ...
          <h2 className="text-2xl font-bold">Configurons votre suivi pornographie</h2>
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fade-in">
              <div className="flex items-center gap-3">
                <EyeOff className="h-5 w-5 text-violet-300" />
              ...
              <input
                ...
                onChange={(e) => setSessionsPerDayInput(e.target.value)}
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">Combien de temps par session ? (en minutes)</span>
              </div>
              <input
                type="number"
                min={0}
