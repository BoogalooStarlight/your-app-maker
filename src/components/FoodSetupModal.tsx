...
import { CircleDollarSign, Sandwich } from "lucide-react";
import { Button } from "@/components/ui/button";

export interface FoodSetupData {
  compulsiveMealsPerDay: number;
  weeklySpendEuros: number;
}

interface FoodSetupModalProps {
  onComplete: (data: FoodSetupData) => void | Promise<void>;
  isSubmitting?: boolean;
}

export const FoodSetupModal = ({ onComplete, isSubmitting = false }: FoodSetupModalProps) => {
  const [step, setStep] = useState(1);
  const [compulsiveMealsPerDayInput, setCompulsiveMealsPerDayInput] = useState("0");
  const [weeklySpendInput, setWeeklySpendInput] = useState("0");

  const compulsiveMealsPerDay = useMemo(
    () => Math.max(0, Number(compulsiveMealsPerDayInput) || 0),
    [compulsiveMealsPerDayInput],
  );
  const weeklySpendEuros = useMemo(() => Math.max(0, Number(weeklySpendInput) || 0), [weeklySpendInput]);

  const handleComplete = () => {
    void onComplete({
      compulsiveMealsPerDay,
      weeklySpendEuros,
    });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-[#09090b] text-white shadow-2xl animate-scale-in">
        <div className="bg-gradient-to-r from-violet-600/80 to-indigo-600/80 p-8 text-center">
          ...
          <h2 className="text-2xl font-bold">Configurons votre suivi nourriture</h2>
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fade-in">
              <div className="flex items-center gap-3">
                <Sandwich className="h-5 w-5 text-violet-300" />
                <span className="font-medium">Combien de fois par jour mangeais-tu de façon compulsive ?</span>
              </div>
              <input
                type="number"
                min={0}
                step="1"
                value={compulsiveMealsPerDayInput}
                onChange={(e) => setCompulsiveMealsPerDayInput(e.target.value)}
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">Combien dépensais-tu par semaine en nourriture compulsive ? (€)</span>
              </div>
              <input
                type="number"
                min={0}
        ...
        <div className="flex gap-3 p-6 pt-0">
          ...
          <Button
            ...
            disabled={isSubmitting}
          >
            {step < 2 ? "Continuer" : isSubmitting ? "Enregistrement..." : "Commencer"}
          </Button>
        </div>
      </div>
    </div>
