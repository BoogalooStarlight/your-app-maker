...
import { MusicSetupModal, type MusicSetupData } from "@/components/MusicSetupModal";

interface MusicModuleData {
  started_at: string;
  daily_quantity: number | null;
}

const Music = () => {
  const [moduleData, setModuleData] = useState<MusicModuleData | null>(null);
  ...
  const loadModuleData = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .eq("module_slug", "music")
      .eq("is_active", true)
      .limit(1)
      .maybeSingle();

    if (error) {
      console.error("Erreur chargement module music:", error.message);
      setIsLoading(false);
      return;
    }
  ...
  const handleSetupComplete = async (data: MusicSetupData) => {
    ...
    if (error) {
      console.error("Erreur création module music:", error.message);
      setIsSubmittingSetup(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      {!moduleData && <MusicSetupModal onComplete={handleSetupComplete} isSubmitting={isSubmittingSetup} />}
