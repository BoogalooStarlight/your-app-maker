...
const Substances = () => {
  ...
  const loadModuleData = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .eq("module_slug", "substances")
      .eq("is_active", true)
      .limit(1)
      .maybeSingle();

    if (error) {
      console.error("Erreur chargement module substances:", error.message);
      return;
    }
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="mx-auto w-full max-w-[980px] px-4 pb-24 pt-24">
        <section className="rounded-3xl border border-white/10 bg-[#050506] p-5">
          <p className="text-sm uppercase tracking-wide text-white/60">Sans substances depuis</p>
